...
3. Ejecutar desde `backend`: `npm run siem:export`. Programar esa orden periódicamente en el servidor de despliegue; la aplicación no la ejecuta por sí sola.
...
No hay una instancia Splunk ni credenciales configuradas en este repositorio; las pruebas usan un HEC simulado. La IP sí consta en los eventos de seguridad locales y se enviará a Splunk al activar este exportador. Definir permisos, retención y base legal antes de activarlo en producción.
